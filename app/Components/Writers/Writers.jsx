...
const Writers = () => {
  ...
  const nextWriter = () => {
    setCurrentWriter((prev) => (prev === writers.length - 1 ? 0 : prev + 1));
  };

  const prevWriter = () => {
    setCurrentWriter((prev) => (prev === 0 ? writers.length - 1 : prev - 1));
  };
  ...
  const handleContactMethod = (method) => {
    ...
    if (method === 'email') {
      ...
      window.open(`https://wa.me/${writer.whatsapp.replace('+', '')}?text=Hello ${writer.name}, I would like to discuss a writing assignment with you.`, '_blank');
    }
  ...
  return (
    <section className="bg-gradient-to-b from-indigo-50 to-indigo-50 min-h-screen py-12 px-4 sm:px-6" aria-label="Meet Our Writers">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Get help from actual writers</h1>
          <p className="text-lg text-gray-700 max-w-3xl mx-auto">
            Work with verified paper writers who understand academic standards, follow your guidelines,
            and deliver top-quality work without using AI tools.
          </p>
        </div>
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          <div className="md:flex">
            {/* Writer Profile Section */}
            <article className="md:w-2/5 p-8 border-r border-gray-200" aria-label={`Profile of ${writer.name}`}>
              <div className="flex items-center mb-6">
                ...
                <div className="ml-4">
                  ...
                  <div className="flex items-center mt-1">
                    ...
                    <span className="ml-2 text-gray-700">{writer.rating} Rating</span>
                  </div>
                  <span className={`mt-1 text-sm font-medium ${
                    writer.status === "Online now" ? "text-green-600" : 
                    writer.status === "Away" ? "text-yellow-600" : "text-gray-500"
                  }`}>
                    {writer.status}
                  </span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-indigo-50 rounded-lg p-4">
                  ...
                  <p className="text-2xl font-bold text-indigo-700">{writer.successRate}%</p>
                </div>
                <div className="bg-indigo-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600">Others completed</p>
                  <p className="text-2xl font-bold text-indigo-700">{writer.completed}</p>
                </div>
                <div className="bg-indigo-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600">Reviews</p>
                  <p className="text-2xl font-bold text-indigo-700">{writer.reviews}</p>
                </div>
                <div className="bg-indigo-50 rounded-lg p-4">
                  <p className="text-sm text-gray-600">Degree</p>
                  <p className="text-lg font-bold text-indigo-700">{writer.degree}</p>
                </div>
              </div>

              <div className="mb-8">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Best Competences</h3>
            ...
            <section className="md:w-3/5 p-8" aria-label="Writer Review and About">
              <div className="mb-10">
                <h3 className="text-xl font-bold text-gray-900 mb-4">Recent review about this writer</h3>
                <blockquote className="bg-indigo-50 rounded-xl p-6">
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-sm font-medium text-gray-600">User ID: {writer.review.userId}</span>
                    <div className="flex text-yellow-400" aria-hidden="true">
                      {[...Array(5)].map((_, i) => (
                  ...
                  <p className="text-gray-700 italic">"{writer.review.text}"</p>
                </blockquote>
              </div>

              <div className="mb-10">
                <h3 className="text-xl font-bold text-gray-900 mb-4">About writer</h3>
                <p className="text-gray-700">
                  {writer.name} is a highly skilled academic writer with a {writer.degree.toLowerCase()} and extensive experience in their field. 
                  They have successfully completed over {writer.completed} papers with a {writer.successRate}% satisfaction rate. 
                  Specializing in {writer.competences.slice(0, 2).join(" and ")}, they bring deep subject expertise and 
                  meticulous attention to detail to every assignment. Their work consistently receives praise for its 
                  originality, depth of research, and adherence to academic standards.
                </p>
              </div>
          ...
          {writers.map((writer, index) => (
            <button
              ...
              className={`flex flex-col items-center p-3 rounded-lg ${
                currentWriter === index ? 'bg-indigo-100 border-2 border-indigo-500' : 'bg-gray-100'
              }`}
            >
              <Image 
                src={writer.image} 
              ...
              />
              <span className="text-sm font-medium">{writer.name.split(' ')[0]}</span>
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <div className="text-sm text-gray-600">{writer.whatsapp}</div>
                </div>
              </button>
            </div>
